...
var _ = framework.KubeDescribe("Projected", func() {
	...
	It("should be consumable from pods in volume [Conformance] [Volume]", func() {
		doProjectionE2EWithoutMapping(f, nil /* default mode */, "projected-secret-test-"+string(uuid.NewUUID()), nil, nil)
	})

	It("should be consumable from pods in volume with defaultMode set [Conformance] [Volume]", func() {
		defaultMode := int32(0400)
		doProjectionE2EWithoutMapping(f, &defaultMode, "projected-secret-test-"+string(uuid.NewUUID()), nil, nil)
	})
	...
	It("should be able to mount in a volume regardless of a different secret existing with same name in different namespace [Volume]", func() {
		...
		doProjectionE2EWithoutMapping(f, nil /* default mode */, secret2.Name, nil, nil)
	})
...
func doProjectionE2EWithoutMapping(f *framework.Framework, defaultMode *int32, secretName string, fsGroup *int64, uid *int64) {
	var (
		volumeName      = "projected-secret-volume"
		volumeMountPath = "/etc/projected-secret-volume"
...
func doProjectionE2EWithMapping(f *framework.Framework, mode *int32) {
	var (
