...
func (s *projectedVolumeMounter) collectData() (map[string]volumeutil.FileProjection, error) {
	...
	for _, source := range s.source.Sources {
		if source.Secret != nil {
			...
			secretapi, err := s.plugin.getSecret(s.pod.Namespace, source.Secret.SecretName)
			//secretapi, err := kubeClient.Core().Secrets(s.pod.Namespace).Get(source.Secret.SecretName, metav1.GetOptions{})
			if err != nil {
...
func getVolumeSource(spec *volume.Spec) (*v1.Projections, bool, error) {
	var readOnly bool
	var volumeSource *v1.Projections
